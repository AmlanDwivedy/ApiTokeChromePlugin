...
const DOM = {
  ...
  filterByTabs_styleElement: document.getElementById(
    "filterByTabs_styleElement"
  ),
  filterByDirection_styleElement: document.getElementById(
    "filterByDirection_styleElement"
  ),
  filterByType_styleElement: document.getElementById(
    "filterByType_styleElement"
  ),
  filterByMethod_styleElement: document.getElementById(
    "filterByMethod_styleElement"
  ),
  filterByStatus_styleElement: document.getElementById(
    "filterByStatus_styleElement"
  ),
...
var t = document.getElementById(
  chrome.extension.getBackgroundPage().currentSettings.o_live_output
); // default output type (formatted/raw)
...
var generate_dom = {
  shouldPrint: function (details) {
    ...
    for (
      let i = 0;
      i <
      chrome.extension.getBackgroundPage().currentSettings.o_live_direction
        .length;
      i++
    ) {
      if (
        ...
        chrome.extension
          .getBackgroundPage()
          .currentSettings.o_live_direction[i].replace(/o_live_direction_/, "")
      ) {
        for (
          let i = 0;
          i <
          chrome.extension.getBackgroundPage().currentSettings.o_live_type
            .length;
          i++
        ) {
          if (
            ...
            chrome.extension
              .getBackgroundPage()
              .currentSettings.o_live_type[i].replace(/o_live_type_/, "")
          ) {
            return true;
          }
        }
    ...
    if (type === "o_live_output_formatted") {
      ...
      if (details.requestHeaders) {
        DOM.dataArea.innerHTML +=
          ...
          "\n\n";
        details.requestHeaders.sort(function (a, b) {
        ...
        DOM.dataArea.innerHTML +=
          "Status: " +
          row.dataset.statusCode +
          " - " +
          httpStatusCodes[row.dataset.statusCode] +
          "\n\n";
        details.responseHeaders.sort(function (a, b) {
          return a.name > b.name ? 1 : b.name > a.name ? -1 : 0;
        });
  ...
  stream: function (n, details, type = null) {
    ...
    if (details.statusCode) {
      ...
      html +=
        '<td class="status ' +
        getStatusCodeStyle(details.statusCode) +
        '" title="' +
        details.statusCode +
        ": " +
        httpStatusCodes[details.statusCode] +
        '">' +
        details.statusCode +
        "</td>";
    } else {
    ...
    if (
      document.querySelectorAll(
        '[data-request-id="' + details.requestId + '"][data-status-code="null"]'
      ).length === 1
    ) {
      document.querySelectorAll(
        '[data-request-id="' + details.requestId + '"][data-status-code="null"]'
      )[0].dataset.statusCode = details.statusCode;
    }
...
function apply_filters(element, type = null) {
  ...
  for (let i = 0; i < DOM[element.name].length; i++) {
    if (DOM[element.name][i].checked) {
      filters +=
        ":not([" + data_name + '="' + DOM[element.name][i].value + '"])';
    }
  }

  if (filters != "") {
    DOM[element.name + "_styleElement"].innerHTML =
      "tbody tr" + filters + "{display:none}";
  } else {
    DOM[element.name + "_styleElement"].innerHTML = "";
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  document.body.classList.add(
    chrome.extension.getBackgroundPage().currentSettings.o_theme
  );
  if (
    chrome.extension.getBackgroundPage().currentSettings.o_live_donation ===
    "o_live_donation_hide"
  ) {
